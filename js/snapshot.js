...
App.Snapshot = {
    ...
    render(feature) {
        ...
        if (!feature.geometry) {
            if(mapDiv) mapDiv.innerHTML = '<div class="flex items-center justify-center h-full text-gray-500">No geometry</div>';
            return;
        }

        try {
            // Ensure the map container is clean if it was previously text-filled
            if (mapDiv && mapDiv.firstChild && mapDiv.firstChild.nodeType !== Node.ELEMENT_NODE) mapDiv.innerHTML = '';
            ...
            if(mapDiv) mapDiv.innerHTML = '<div class="flex items-center justify-center h-full text-gray-500">Error rendering map</div>';
        }
    }
};